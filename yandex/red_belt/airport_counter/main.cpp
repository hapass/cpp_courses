...
public:
  static const uint32_t AirportCount = static_cast<uint32_t>(TAirport::Last_);

  AirportCounter() {
    for (uint32_t i = 0; i < AirportCount; i++) items[i] = make_pair(static_cast<TAirport>(i), 0);
  }

  template <typename TIterator>
  AirportCounter(TIterator begin, TIterator end): AirportCounter() {
    for (auto it = begin; it != end; it++) items[static_cast<uint32_t>(*it)].second++;
  }
  ...
  using Items = array<Item, AirportCount>;
