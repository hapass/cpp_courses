...
class RangeIterator {
public:
  RangeIterator(Iterator b, Iterator e): begin_(b), end_(e) {}
  ...
  Paginator(Iterator b, Iterator e, size_t page_size)
    ...
    while (length > 0) {
      if (length >= page_size_) {
        ranges.push_back(RangeIterator(begin_ + offset, begin_ + offset + page_size_));
        length -= page_size_;
        offset += page_size_;
      }
      else {
        ranges.push_back(RangeIterator(begin_ + offset, end_));
        offset = length;
        length = 0;
      }
      total_page_count_++;
...
private:
  ...
  Iterator begin_;
  Iterator end_;
  vector<RangeIterator<Iterator>> ranges;
};
