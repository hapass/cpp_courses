...
void RunProgram(istream& input, ostream& output) {
    ...
    map<string, int> operation_values = { {"+", 0}, {"-", 0}, {"*", 1}, {"/", 1} };
    deque<string> expression;
    int prev_operation_value = 1;
    expression.push_back(initial_value);
    while(operations_count-- > 0) {
        string operation;
        int value = 0;
        input >> operation >> value;
        int operation_value = operation_values.at(operation);
        if (operation_value - prev_operation_value > 0) {
        ...
        prev_operation_value = operation_value;
    }
